...
def index(request):
  ...
  getFinancialData(memuser.seq_no,'RE',context)
  return render(request, "statementMajor/statementRE.html",context)
...
def setSelectedYear(request):
  ...
  if selectedKejung==False:
    context['selectedYear'] =selectedYear
    context['statementList'] =getStatementList(memuser.user_id,'RE',selectedYear)
  else:
    context['statementList'] =getStatementList(memuser.user_id,'RE',selectedYear)
