...
class ActionCtx_ATECRCCA001R06:
    def __init__(self):
...
class ActionItem_ATECRCCA001R06:
    def __init__(self):
    ...
    def readXML(self, node):
        ...
        subnode = node.find("mrntTxprNm")   # 가맹점 상호
        if subnode is None:
            self.m_mrntTxprNm= ""
        else:
            self.m_mrntTxprNm = subnode.text
        subnode = node.find("totaTrsAmt")               # 합계
        self.m_totaTrsAmt = self._xmlTextToLong(subnode)
        ...
        self.m_rowSeq = self._xmlTextToInt(subnode)
        ...
        self.m_splCft = self._xmlTextToLong(subnode)

        subnode = node.find("busnCrdcTrsBrkdPrhYr")
        self.m_busnCrdcTrsBrkdPrhYr = self._xmlTextToInt(subnode)
        ...
        self.m_tip = self._xmlTextToLong(subnode)
        ...
        self.m_vaTxamt = self._xmlTextToLong(subnode)
        ...
        if subnode is None:
            ...
            self.m_prhQrt = self._xmlTextToInt(subnode)
...
REFERER_URL = "https://tecr.hometax.go.kr/websquare/websquare.html?w2xPath=/ui/cr/c/b/UTECRCB023.xml"
class JsHometax_Screen_UTECRCB023:
  ...
  def requestPermission(self):
      ...
      rst = jsHometax.requestPermission(REFERER_URL, "tecr", "UTECRCB023", None, None, None, reqperm_errorCd, reqperm_errorMsg)
      ...
      # print(rst)
      if rst != 1:    return rst
      rst = jsHometax.requestPermission(REFERER_URL, "tecr", "UTECRCB023", "hometax.go.kr", sbSSOToken, sbuserClCd, reqperm_errorCd, reqperm_errorMsg)
      # print(rst)
      if rst != 1:    return rst
      
      return 1
    
  def action_ATECRCCA001R06(self,actionctx, startdate, enddate):
      refererUrl = "https://tecr.hometax.go.kr/wqAction.do?actionId=ATECRCCA001R06&screenId=UTECRCB023&popupYn=false&realScreenId="
      url = "https://tecr.hometax.go.kr/wqAction.do?actionId=ATECRCCA001R06&screenId=UTECRCB023&popupYn=false&realScreenId="
      ...
      # 1. First Data
      postdata = "<map id=\"ATECRCCA001R06\"><tin>" + jsHometax.mLoginSystem_Main.m_tin + "</tin><trsDtRngStrt>" + startdate + "</trsDtRngStrt><trsDtRngEnd>" + enddate + "</trsDtRngEnd><prhTxamtDdcYn>all</prhTxamtDdcYn><sumTotaTrsAmt/><resultCd/></map>"
      # print(postdata)
      ...
      xmlrawnode_pageInfoVO = xmlnodelst[2]
  ...
  def action_ATECRCCA001R06_getItems(self,actionctx, page,cursor,workyear,strChkDate,HometaxID,HometaxPW,flagSeqNo,biz_no,biz_name):
    ...
    #   print(strResData)
      ...
      for node in map_elements:
          item = ActionItem_ATECRCCA001R06()
          item.readXML(node)
        ...
        #   if strChkDate=='6' and int(item.m_aprvDt[5:7])<7: item.m_aprvDt = item.m_aprvDt[0:5]+"07-01"
          ...
          sql += f" VALUES ('{workyear}','{tran_dt}','{i}','{strChkDate}','{item.m_bcNm}','{item.m_tfbNm}','{HometaxID}','{HometaxPW}"
          sql += f"','{biz_no}','{flagSeqNo}','{item.m_rowSeq}','{biz_name}','3','{item.m_aprvDt}','{item.m_crcmClNm}','{item.m_busnCrdCardNoEncCntn}"
          sql += f"','{item.m_mrntTxprDscmNoEncCntn}','{mrntTxprNm}','{item.m_splCft}','{item.m_vaTxamt}','{item.m_tip}','{item.m_totaTrsAmt}','{item.m_bmanClNm}"
          sql += f"','{item.m_ddcYnNm}','{item.m_vatDdcClNm}','57','Y','830','소모품비','','"+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"','Y','','','','','')"
...
def _xmlTextToInt(node, nullval):
    if node is None:
        return nullval
    else:
        try:
            val = int(node.InnerText)
            return val
        except ValueError:
            return nullval
        
def _xmlTextToInt(node):
    return _xmlTextToInt(node, -1)

def _xmlTextToLong(node, nullval):
    if node is None:
        return nullval
    else:
        try:
